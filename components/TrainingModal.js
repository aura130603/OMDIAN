...
export default function TrainingModal({ training, onSave, onClose }) {
  ...
  const [formData, setFormData] = useState({
    ...
    tanggalSelesai: '',
    jamPelajaran: '',
    sertifikat: '' // string untuk URL sertifikat
  })
  const [error, setError] = useState('')
  const [loading, setLoading] = useState(false)
  ...
  useEffect(() => {
    if (training) {
      setFormData({
        ...
        tanggalSelesai: training.tanggalSelesai || '',
        jamPelajaran: training.keterangan || '',
        sertifikat: training.sertifikat || ''
      })
    }
  }, [training])
  ...
  const handleSubmit = async (e) => {
    ...
    // Opsional: validasi format URL sertifikat
    if (formData.sertifikat && !/^https?:\/\/.+/.test(formData.sertifikat)) {
      setError('Link sertifikat tidak valid. Harus diawali dengan http:// atau https://')
      setLoading(false)
      return
    }

    try {
      // Map jamPelajaran ke keterangan untuk backend
      const dataToSave = {
        ...formData,
        keterangan: formData.jamPelajaran
      }
  ...
  return (
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-content">
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label htmlFor="tema" className="form-label">Tema Diklat/Workshop/Seminar *</label>
            <input
              type="text"
              id="tema"
              name="tema"
          ...
          <div className="form-group">
            <label htmlFor="jamPelajaran" className="form-label">Jam Pelajaran</label>
            <input
              type="number"
              id="jamPelajaran"
              name="jamPelajaran"
          ...
          <div className="form-group">
            <label htmlFor="sertifikat" className="form-label">
              Link Sertifikat (Google Drive)
              <span
                className={`status-badge ${formData.sertifikat ? 'status-active' : 'status-inactive'}`}
                style={{ marginLeft: '10px', fontSize: '11px' }}
              >
                {formData.sertifikat ? 'Sudah Ada' : 'Belum Ada'}
              </span>
            </label>
            <input
              type="url"
            ...
            />
            <div style={{ fontSize: '12px', color: 'var(--text-medium)', marginTop: '5px' }}>
              <strong>Catatan:</strong> Sertifikat opsional. Pastikan link Google Drive dapat diakses publik.
            </div>
          </div>
